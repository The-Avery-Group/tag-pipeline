...
async function uploadOne(opportunityKey, file, onChunk) {
  const validationError = validateOpportunityReferenceFile(file)
  if (validationError) throw new Error(validationError)
  const prepared = await createOpportunityReferenceUpload(opportunityKey, file)
...
export async function uploadOpportunityReferenceFiles(opportunityKey, files, onProgress) {
  ...
  try {
    for (let index = 0; index < selected.length; index += 1) {
      ...
      uploaded.push(await uploadOne(opportunityKey, file, (chunkSize) => {
    ...
    if (uploaded.length) {
      await rollbackUploadedItems(opportunityKey, uploaded).catch(() => {})
    }
    throw error
  }
  return uploaded
