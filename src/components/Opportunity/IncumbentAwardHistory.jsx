...
function AgencyTooltip({ active, payload }) {
  ...
  return <div className={styles.incumbentChartTooltip}><strong>{agency.name}</strong><div className={styles.incumbentTooltipRow}><i style={{ background: agency.color }} /><span>{agency.count} prime contract{agency.count === 1 ? '' : 's'}</span><strong>{agency.percent.toFixed(1)}%</strong></div><span>{fullCurrency(agency.value)} total award value</span></div>
}

export default function IncumbentAwardHistory({ incumbentUEI }) {
  ...
  const doughnut = [...leading, ...(remaining.count ? [{ name: 'Other agencies', ...remaining }] : [])]
    .map((item, index) => ({ ...item, percent: agencyCount ? item.count / agencyCount * 100 : 0, color: CHART_COLORS[index % CHART_COLORS.length] }))

  return <div className={styles.incumbentHistory}>
    <button type="button" className={styles.incumbentHistoryHeader} onClick={() => setOpen((value) => !value)} aria-expanded={open}><span><strong>Incumbent award history</strong><small>USAspending.gov · last 5 years</small></span><span>{open ? '⌃' : '⌄'}</span></button>
      ...
      {loading ? <div className="text-xs text-muted">Loading incumbent award history…</div>
        ...
        : data && <>
          ...
          <div className={styles.incumbentHistoryContent}>
            ...
            <div className={styles.incumbentAgencyPanel}><div className={styles.incumbentChartTitle}>Prime contracts by agency</div>{doughnut.length === 0 ? <div className="text-xs text-muted">No agency data.</div> : <div className={styles.incumbentAgencyLayout}><div className={styles.incumbentDoughnut}><ResponsiveContainer width="100%" height="100%"><PieChart><Tooltip allowEscapeViewBox={{ x: true, y: true }} wrapperStyle={{ zIndex: 20 }} content={<AgencyTooltip />} /><Pie data={doughnut} dataKey="count" nameKey="name" innerRadius={55} outerRadius={90} label={({ percent }) => `${Number(percent).toFixed(0)}%`} labelLine={false}>{doughnut.map((item) => <Cell key={item.name} fill={item.color} />)}</Pie></PieChart></ResponsiveContainer></div><div className={styles.incumbentHistoryAgencies}>{doughnut.map((agency) => <div key={agency.name} title={`${agency.count} prime contract${agency.count === 1 ? '' : 's'}`}><span>{agency.name}</span><strong>{agency.percent.toFixed(1)}%</strong></div>)}</div></div>}</div>
          </div>
