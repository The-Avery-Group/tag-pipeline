...
export default function AgencyIntelligence() {
  ...
  const progressPercent = usageRun?.totalOrders ? Math.min(100, Math.round((Number(usageRun.processedOrders || 0) / usageRun.totalOrders) * 100)) : 8
        ...
        {!selectedAgency ? <div className={styles.emptyDetail}><strong>Select an agency</strong><span>Choose an agency from the pipeline or search USAspending to review the contract vehicles it uses.</span></div> : <>
          ...
          <div className={styles.resultsScroll}>
            {viewMode === 'usage' ? <section className={styles.vehicleSection}>
              ...
              {usageRun && <div className={styles.progressBlock}><div><span>{usageRun.phase === 'resolving' ? 'Resolving contract vehicle names' : 'Building the agency-wide vehicle aggregate'}</span><b>{usageRun.totalOrders ? `${Number(usageRun.processedOrders || 0).toLocaleString()} of ${Number(usageRun.totalOrders).toLocaleString()} orders` : 'Preparing order records'}</b></div><div className={styles.progressTrack}><span style={{ width: `${progressPercent}%` }} /></div></div>}
