...
function resourceTypeLabel(value) {
  return clean(value) || 'External link'
}
    ...
    <div className={`page-body ${styles.page}`}>
      ...
      {(detail.links?.length > 0 || detail.attachments?.length > 0) && <Card eyebrow="Resources" title="Attachments and links" count={(detail.links?.length || 0) + (detail.attachments?.length || 0)}>
        {detail.attachments?.length > 0 && <div className={styles.resourceGroup}>
          ...
          <div className={styles.resourceList}>{detail.attachments.map((file, index) => <article key={file.sourceUrl || index}><div><strong>{file.fileName}</strong><span><span className={styles.resourceType}>{resourceTypeLabel(file.resourceTypeLabel)}</span>{file.byteSize ? `${Math.ceil(file.byteSize / 1024)} KB · ` : ''}{file.archiveStatus === 'archived' ? 'Preserved in SAM.gov Archive' : file.archiveStatus === 'moved' ? 'Moved into opportunity workspace' : file.archiveStatus === 'failed' ? 'Archive failed' : 'Awaiting archive'}</span>{file.errorMessage && <span className={styles.fileError}>{file.errorMessage}</span>}</div><div className={styles.resourceActions}>{file.webUrl && <a className="btn" href={file.webUrl} target="_blank" rel="noreferrer">Open saved file</a>}<a className="btn" href={file.sourceUrl} target="_blank" rel="noreferrer">Open original</a></div></article>)}</div>
        </div>}
        {detail.links?.length > 0 && <div className={styles.resourceGroup}><h3>External links</h3><div className={styles.resourceList}>{detail.links.map((link) => <article key={link.url}><div><strong>{link.label}</strong><span><span className={styles.resourceType}>{resourceTypeLabel(link.resourceTypeLabel)}</span>{linkHost(link.url)}</span><CopyValue value={link.url} label="link" /></div><a className="btn" href={link.url} target="_blank" rel="noreferrer">Open original</a></article>)}</div></div>}
      </Card>}
