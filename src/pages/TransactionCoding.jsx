...
export default function TransactionCoding({ toast }) {
  ...
  const saveRule = async () => {
    ...
    try {
      ...
      setRuleDraft(null)
      notify(toast, result.warning || (existing ? 'Rule updated.' : 'Rule saved to the workbook.'), result.warning ? 'warning' : 'success')
    }
  ...
  const removeRule = async (rule) => {
    ...
    try {
      ...
      notify(toast, result.warning || 'Rule deleted.', result.warning ? 'warning' : 'success')
    } catch (ruleError) { setError(ruleError.message) }
    finally { setBusy('') }
  }
